...
// Fonction qui met la date au bon format
      ...
      useEffect(() => {
        if (selectedIndex==1) {
          ...
          async function recupPastVisit() {
            const response = await fetch('http://10.2.3.7:3000/get-past-visit', {
              method: 'POST',
  ...
  // Sinon => visites à venir
          ...
          async function recupFuturVisit() {
            const response = await fetch('http://10.2.3.7:3000/get-futur-visit', {
              method: 'POST',
              headers: {'Content-Type':'application/x-www-form-urlencoded'},
              body: `token=${props.searchToken}`
            })
    ...
    return (
        ...
        />
          ...
          {cardList}
            ...
            <Button title="Add monument BDD" onPress={() => fetch ("http://10.2.3.7:3000/save-monument")}/>
